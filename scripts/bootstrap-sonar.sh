...
# Helper: was the default password already changed on a previous run?
default_works() {
...
if default_works; then
  info "Changing default admin password ..."
  curl -sf -u "${DEFAULT_USER}:${DEFAULT_PASS}" -X POST \
...
elif newpass_works; then
  warn "Admin password already changed on a previous run — reusing it."
  AUTH="${DEFAULT_USER}:${NEW_PASS}"
else
  fail "Cannot authenticate to Sonar with default OR configured password."
fi
...
# --- 4. Register the Jenkins webhook (so the quality gate reports back) ------
